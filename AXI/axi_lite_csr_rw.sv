...
// Core can read and reset bits by writing a 1.
// Registers can be read and written by AXI.


// (w1c) write 1 to clear
module axi_lite_csr_rw #(
    ...
    // core interface reads bits and clears them by writing 1
    input  logic [AXI_DATA_WIDTH-1:0] core_i [NUM_REGS-1:0],
    output logic [AXI_DATA_WIDTH-1:0] core_o [NUM_REGS-1:0]
);
    ...
    // Drive coutput to core
    always_comb
        for (int i = 0; i < NUM_REGS; i++)
            core_o[i] = registers[i];
    ...
    always_ff @(posedge clk) begin
        if (reset) begin
            ...
            for (int i = 0; i < NUM_REGS; i++) begin
                // --- AXI Writes Bit (Priority 1)
                if (reg_we && (write_index == i)) begin
                    for (int j = 0; j < 32; j++) begin
                        if (s_axi.wstrb[j/8]) begin
                            registers[i][j] <= s_axi.wdata[j];
                        end
                    end
                // --- Core Clears Bits (w1c) (Priority 2)
                end else begin
