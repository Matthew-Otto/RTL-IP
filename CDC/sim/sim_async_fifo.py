...
async def random_stimulus(dut):
    ...
    # start system clock
    cocotb.start_soon(Clock(dut.clk_in, 7, unit="ns").start())
    cocotb.start_soon(Clock(dut.clk_out, 13, unit="ns").start())
    await reset(dut)
...
def test_runner():
    ...
    sources = [proj_path / "async_fifo.sv"]

    mem_path = proj_path.parents[0] / "Memory"
    sources += list(mem_path.rglob("*.sv"))
    rst_path = proj_path.parents[0] / "Resets"
    sources += list(rst_path.rglob("*.sv"))
